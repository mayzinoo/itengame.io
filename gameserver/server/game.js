...
Game.prototype.placeBet = function(user, betAmount, autoCashOut, pay_amt, profit, eth_value, callback) {
    console.log('Bet',betAmount);    
    console.log('Pay Amount', autoCashOut);
    ...
    db.placeBet(betAmount, autoCashOut, pay_amt, profit, eth_value,  user.id, self.gameId, function(err, playId) {
        self.pendingCount--;
        ...
        if (err) {
            ...
            var index = self.joined.insert({ user: user, bet: betAmount, autoCashOut: autoCashOut, pay_amt: pay_amt, profit: profit, playId: playId, status: 'ENDED' });
            
            //For My Bets
            //var index = self.mybets.insert({ user: user, bet: betAmount, autoCashOut: autoCashOut, pay_amt: pay_amt, profit: profit, playId: playId, status: 'ENDED' });
...
Game.prototype.reducebalance = function(user, balance, callback) {    
    var self = this;  
   
    db.reduce_balance(user.id, balance,  function(err, playId) {
...
Game.prototype.logout = function(user, username, callback) {    
    var self = this;  
   
    db.logout(user.id, username,  function(err) {

        if (err) {
            if (err.code == '23514') // constraint violation
                return callback('NOT_ENOUGH_MONEY');

            console.log('[INTERNAL_ERROR] could not logout, got error: ', err);
            callback(err);
        } else {           
...
Game.prototype.readdbalance = function(user, balance, bet, callback) {    
    var self = this; 
    var bet = bet;
    var autoCashOut = 0;
    var new_bet = bet;
    ...
    db.readd_balance(user.id, balance, function(err, playId) {
        if (err) {
            if (err.code == '23514') // constraint violation
                return callback('NOT_ENOUGH_MONEY');
