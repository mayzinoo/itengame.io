...
exports.stoptime = function(callback) {
    query('SELECT stop_date FROM ratestop ', function(err, result) {
        if (err) return callback(err);
        if (result.rows.length === 0)
            return callback('USER_DOES_NOT_EXIST');
...
exports.logout = function(userId, username, callback) {
   query('UPDATE sessions SET expired = now() WHERE user_id = $1 AND expired > now()', [userId], callback);
};

exports.reduce_balance = function(userId, balance, callback) {
    console.log('db balance', balance);
    //query('UPDATE users SET balance = $2  WHERE id = $1', [userId, balance], callback);
   getClient(function(client, callback) {
      var tasks = [
        function(callback) {
          client.query('UPDATE users SET balance = $2  WHERE id = $1',
...
exports.readd_balance = function(userId, balance, callback) {
...
exports.updatebalance = function(bet, autocacheout,  payout, profit, eth_value, userId, gameId, callback) {
    console.log('db eth', eth_value);
    getClient(function(client, callback) {
      var tasks = [
        function(callback) {
          client.query('UPDATE users SET balance = $1 WHERE id = $2',  [eth_value, userId] , callback);
        },         
      ];
...
exports.placeBet = function(bet, autocacheout,  payout, profit, eth_value, userId, gameId, callback) {
    console.log('db eth', eth_value);
    ...
    getClient(function(client, callback) {
      var tasks = [
        function(callback) {
          client.query('UPDATE users SET balance = $1 WHERE id = $2',
            [eth_value, userId], callback);
        },
        
        function(callback) {
          client.query(
            'INSERT INTO plays(user_id, game_id, bet, auto_cash_out ,cash_out, profit) VALUES($1, $2, $3, $4, $5, $6) RETURNING id',
            [userId, gameId, bet, autocacheout, payout, profit], callback);
        },
        function(callback) {
            client.query('SELECT * FROM users WHERE id = $1', [userId], function(err, result) {
                ...
                client.query('SELECT * FROM referral where user_id = $1', [username], function(err, res) {
                    console.log(err);
                    console.log(res)
                    console.log('reffered user found');
                    if(err) return callback(err);
                    if(res.rows.length>0){
                        let referred_by= res.rows[0]['referred_by'];
                        client.query('SELECT * FROM users WHERE id = $1', [referred_by], function(err, result) {
                            if (err) return callback(err);
                            let rate = result.rows[0]['rate'];
                            let amt = (bet*rate)/100;
                            client.query('INSERT INTO referral_earnings(user_id,user_id_benefit,old_bet,created_on,user_username) values($1,$2,$3,$4,$5)', 
                            [userId,referred_by,amt,new Date(),username], callback);
                        });
                    }else{
                    callback(null, []);
                    }
...
exports.getGameHistory = function(callback) {
    ...
    var sql =
        'SELECT plays.id id, created, ' +
        '     (SELECT array_to_json(array_agg(pv)) ' +
        '        FROM (SELECT username, bet, (100 * cash_out / bet) AS stopped_at, bonus ' +
        '              FROM plays JOIN users ON user_id = users.id ) pv) player_info ' +
        'FROM plays ' +
        'ORDER BY plays.id DESC LIMIT 10';
