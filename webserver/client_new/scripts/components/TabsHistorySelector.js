...
define(['react', 'components/Chat', 'components/GamesLog', 'components/Players', 'components/SettingsSelector', 'components/StrategyEditor', 'stores/TabsSelectorStore', 'actions/TabsSelectorActions'], function (React, ChatClass, GamesLogClass, PlayersClass, SettingsSelectorClass, StrategyEditorClass, TabsSelectorStore, TabsSelectorActions) {
  ...
  return React.createClass({
    ...
    render: function render() {
      ...
      var autoBetTab,
          playersTab,
          tabCols,
          tabCount = 2; //Show the players tab when we are on mobile

      if (this.props.isMobileOrSmall) tabCount++; //Show the auto widget when the controls are set to small
      ...
      return D.div({
        id: 'tabs-inner-container',
        className: 'log-chat-tabs-container m-hide'
      }, D.div({
        className: 'tab-container noselect'
      }, D.ul({
        className: ''
      }, //     D.li({
      //     className: 'tab d-show ' + tabCols + ' noselect' + ((this.state.selectedTab === 'gamesLog') ? ' tab-active' : ''),
      //     },
      //     D.a(null, 'History'),
      //     D.li({
      //             className: 'tab d-hide ' + tabCols + ' noselect' + ((this.state.selectedTab === 'gamesLog') ? ' tab-active' : ''),
      //             onClick: this._selectTab('gamesLog')
      //         },
      //         D.a(null, 'History')
      //     )
      // ),
      D.li({
        className: 'tab d-hide ' + tabCols + ' noselect' + (this.state.selectedTab === 'chat' ? ' tab-active' : ''),
        onClick: this._selectTab('chat')
      }, D.a(null, 'Chat')), D.li({
        className: 'tab ' + tabCols + ' noselect' + (this.state.selectedTab === 'chat' ? ' tab-active' : ''),
        onClick: this._selectTab('chat')
      }, D.a(null, 'Chat')) // playersTab,
      // autoBetTab
      ) // D.div({
      //         className: 'tab-settings noselect' + ((this.state.selectedTab === 'settings') ? ' tab-active' : ''),
      //         onClick: this._selectTab('settings')
      ...
      // )
        ...
        className: 'widget-container ' + contClass
      }, widget));
    }
  });
});