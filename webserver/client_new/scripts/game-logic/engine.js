define([
    ...
    function Engine() {
        ...
        self.ws.on('game_crash', function(data) {
            ...
            for (var user in data.bonuses) {
                ...
                if (self.username === user) {
                    self.balance += data.bonuses[user];
                }
            }
        ...
        self.ws.on('game_starting', function(info) {
            console.log('game-staring gmae-starting');
            self.playerInfo = {};
            self.joined = [];
        ...
        self.ws.on('player_bet', function(data) {
            console.log('here engine player bet', data);
            ...
            self.joined.splice(data.index, 0, data);
        ...
        self.ws.on('cashed_out', function(resp) {
            ...
            if (self.username === resp.username) {
                self.cashingOut = false;
                self.balance += self.playerInfo[resp.username].bet * resp.stopped_at / 100;
            }
            ...
            requestOtt(function(err, ott) {
                ...
                self.ws.emit('join', { ott: window.DEV_OTT? window.DEV_OTT : ott },
                    function(err, resp) {
                        ...
                        console.log('bbbbbbb', resp.balance);
                        //self.chat = resp.chat;
                        ...
                        self.role = resp.role;
                        //console.log('rrrrrrrrrrr', self.role);
    ...
    Engine.prototype.logout = function(username) {
        
        this.ws.emit('logout', username, function(error) {
    ...
    Engine.prototype.reducebalance = function(balance) {
        var new_balance = balance;
        this.ws.emit('reduce_balance', new_balance, function(error) {

            if (error) {
                console.warn('reducebalance error: ', error);
    ...
    Engine.prototype.readdbalance = function(balance, cclicks) {
    var new_balance = balance;
	var bet = cclicks;
        this.ws.emit('readd_balance', new_balance, bet,  function(error) {

            if (error) {
                console.warn('readdbalance error: ', error);
     ...
     */
    Engine.prototype.bet = function(amount, autoCashOut, eth_value, profitvalue,  callback) {
        var autocacheout = 0;
        var payout = autoCashOut;
        var bet = amount;     
        var profit = (Number(profitvalue)) - (Number(bet)); 
        var eth_value = eth_value;

        this.nextBetAmount = 10;
        this.nextAutoCashout = 100;
        this.placingBet = true;

        //if (this.gameState === 'STARTING')
            return this.doBet(bet, autocacheout, payout, profit, eth_value, callback);
    ...
    Engine.prototype.doBet =  function(bet, autocacheout, payout, profit, eth_value,  callback) {
        console.log('bet',bet);
        console.log('payout', payout);
        console.log('profit', profit);
        console.log('ethvalue', eth_value)
        var self = this;

        this.ws.emit('place_bet',bet, autocacheout, payout, profit, eth_value, function(error) {
    ...
    AppDispatcher.register(function(payload) {
        ...
        switch(action.actionType) {
            ...
            case AppConstants.ActionTypes.PLACE_BET:
                EngineSingleton.bet(action.bet, action.cashOut, action.eth_value, action.profitvalue);
                break;

            case AppConstants.ActionTypes.START_GAME:
                EngineSingleton.reducebalance(action.balance);
                break; 
            
            case AppConstants.ActionTypes.LOGOUT:
                EngineSingleton.logout(action.username);
                break;    
 
            case AppConstants.ActionTypes.END_GAME:
                EngineSingleton.readdbalance(action.balance, action.cclicks);
                break;         
