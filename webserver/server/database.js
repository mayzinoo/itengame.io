...
exports.expireSessionsBysessionId = function(sessionId, callback) {
    ...
    query('UPDATE sessions SET expired = now() WHERE id = $1 AND expired > now()', [sessionId], callback);
};
...
exports.checkreceiver = function(receiverid,  callback) {    
    query('select * from users where name = lower($1)', [receiverid], function(err, data) {
       if (err) return callback(err);
...
exports.getitenrate = function(callback) {    
    query('SELECT * FROM itenrate order by id desc limit 1', function(err, result) { 
    if (err) 
        console.log('error',err);
        return callback(err);
        if (result.rows.length === 0)
            return callback('USER_DOES_NOT_EXIST');
        var rate = result.rows[0].iten_rate;    
       callback(null, rate);
    });
};
